...
type Filter = 'All' | 'Demo' | 'Prototype';
export default function ActivityScreen() {
  ...
  return <Screen><AppHeader title="Activity" subtitle="Cloud incident history" /><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} />}>
    <View style={styles.filters}>{(['All', 'Demo', 'Prototype'] as const).map((item) => <Pressable key={item} accessibilityRole="button" accessibilityState={{ selected: filter === item }} onPress={() => setFilter(item)} style={[styles.filter, filter === item && styles.filterActive]}><Text style={[styles.filterText, filter === item && styles.filterTextActive]}>{item}</Text></Pressable>)}</View>
...
function IncidentRow({ incident, last }: { incident: CloudIncident; last: boolean }) {
  ...
  return <Pressable accessibilityRole="button" onPress={() => router.push({ pathname: '/incident/[id]', params: { id: incident.id } })} style={({ pressed }) => [styles.incident, !last && styles.incidentBorder, pressed && styles.pressed]}>
    <View style={[styles.incidentIcon, incident.status === 'cancelled' && styles.cancelledIcon]}><Ionicons name={incident.status === 'active' ? 'radio-outline' : incident.status === 'resolved' ? 'shield-checkmark-outline' : 'close-outline'} size={21} color={incident.status === 'active' ? colors.red : colors.navy} /></View>
    <View style={styles.incidentCopy}><View style={styles.titleRow}><Text style={styles.incidentTitle}>SOS {incident.status.replace('_', ' ')}</Text><StatusBadge label={incident.is_demo ? 'Demo' : 'Prototype'} tone={incident.is_demo ? 'neutral' : 'danger'} /></View><Text style={styles.meta}>{formatTime(incident.started_at)} • {duration} • {ackCount === null ? '—' : ackCount} acknowledgements</Text><Text style={styles.source}>{sourceLabel(incident.activation_source)}</Text></View><Ionicons name="chevron-forward" size={18} color={colors.textMuted} />
  </Pressable>;
}
...
const styles = StyleSheet.create({
  content: { paddingHorizontal: spacing.lg, paddingBottom: 32, gap: 18 }, filters: { flexDirection: 'row', padding: 4, borderRadius: 14, backgroundColor: '#EAECF0' }, filter: { flex: 1, minHeight: 40, borderRadius: 10, alignItems: 'center', justifyContent: 'center' }, filterActive: { backgroundColor: colors.white, borderWidth: 1, borderColor: colors.border }, filterText: { color: colors.textSecondary, fontSize: 13, fontWeight: '600' }, filterTextActive: { color: colors.navy }, error: { color: '#B54708', backgroundColor: colors.amberSoft, borderRadius: 10, padding: 11, fontSize: 12 }, dateLabel: { color: colors.textSecondary, fontSize: 10, fontWeight: '800', letterSpacing: 0.8, marginBottom: 9 }, groupCard: { paddingVertical: 2, paddingHorizontal: 16 }, incident: { flexDirection: 'row', alignItems: 'center', minHeight: 94, paddingVertical: 14 }, incidentBorder: { borderBottomWidth: 1, borderBottomColor: colors.border }, pressed: { opacity: 0.62 }, incidentIcon: { width: 42, height: 42, borderRadius: radii.small, backgroundColor: '#E8EEF4', alignItems: 'center', justifyContent: 'center' }, cancelledIcon: { backgroundColor: colors.background }, incidentCopy: { flex: 1, marginHorizontal: 11 }, titleRow: { flexDirection: 'row', alignItems: 'center', gap: 7, flexWrap: 'wrap' }, incidentTitle: { color: colors.text, fontSize: 14, fontWeight: '700', textTransform: 'capitalize' }, meta: { color: colors.textSecondary, fontSize: 10, marginTop: 7 }, source: { color: colors.textMuted, fontSize: 10, marginTop: 4 },
});
